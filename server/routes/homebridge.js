...
// homebridgeRouter.use((request & { headers: { authorization: string } }, response, next: NextFunction) => {
...
homebridgeRouter.get("/dashboard", (request, response) => {
  var configFile = fs.readFileSync('/Users/Mike/Code/homebridge/config.json', 'utf8');

  response.send(configFile);
});

homebridgeRouter.get("/config/rebuild", (request, response) => {
  buildFullConfig();

  response.send("Rebuilding Homebridge Config");
});

homebridgeRouter.post("/restart", (request, response) => {
  io = response.io;
  stopHomebridge();
  startHomebridge();
});

homebridgeRouter.post("/stop", (request, response) => {
  io = response.io;
  stopHomebridge();
});

homebridgeRouter.post("/start", (request, response) => {
  io = response.io;
  startHomebridge();
});

homebridgeRouter.get("/status", (request, response) => {
  response.send({
...
homebridgeRouter.get("/config", (request, response) => {
...
homebridgeRouter.get("/ver", (request, response) => {
...
homebridgeRouter.put("/config", (request, response) => {
  var filePath = homebridgeDir + 'config.json';
...
homebridgeRouter.get("/config/schema", (request, response) => {
...
//TODO: REST:Homebridge Log output
homebridgeRouter.get("/log", (request, response) => {
  var configFile = fs.readFileSync('/Users/Mike/Code/homebridge/config.json', 'utf8');
...
function buildFullConfig() {
  ...
  var homebridgeConfigFile = require(homebridgeConfigsDir + 'homebridge.config.json');
  ...
  configFiles.forEach(fileName => {
    let configFile = require(configDir + fileName);
